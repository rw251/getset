const convertTemplate = require('../../shared/templates/convert.jade');
...
const refreshSynonymUI = () => {
  const html = synonymTemplate({ synonyms: Object.keys(currentCodeSet.s) });
  $synonym.include.list.html(html);
};

const refreshExclusionUI = () => {
  const html = synonymTemplate({ synonyms: Object.keys(currentCodeSet.e) });
...
module.exports = {
  ...
  show: () => {
    defaultController(convertTemplate);
    wireup();
  },
