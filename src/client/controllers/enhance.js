import enhanceComponent from '../components/enhance';
import enhanceResultsComponent from '../components/enhanceResultsAlt';
import ajaxLoaderComponent from '../components/ajaxLoader';
import synonymComponent from '../components/partials/synonyms';
import api from '../scripts/api';
import { getCodeForTerminology, findPos, getSelectionCoords } from '../scripts/utils';
import { getHierarchiesWithExistingCodeList, getHierarchies } from '../scripts/graph-utils';
...
const displayResults = (groups) => {
  const html = enhanceResultsComponent(groups);
...
const refresh = () => {
  ...
  setTimeout(() => {
    ...
    api.codeSearch(terms, currentTerminology)
      .then((data) => {
        const hierarchies = getHierarchiesWithExistingCodeList(
          data.codes,
          currentTerminology,
          terms,
...
const updateFromCodeSet = (codeSet) => {
  api.unmatchedChildren(codeSet, currentTerminology)
    .then((data) => {
      // data is
      //  {
      ...
      data.codes.forEach((code) => {
        currentGroups.codeLookup[getCodeForTerminology(code.clinicalCode, currentTerminology)] = code;
      });
      ...
      currentGroups.notInCodeSetDescendantOfCodeSet = Object
        ...
        .filter(umc => currentGroups.codeLookup[getCodeForTerminology(umc, currentTerminology)])
        .map(umc => ({
          code: getCodeForTerminology(umc, currentTerminology),
          description: currentGroups
            .codeLookup[getCodeForTerminology(umc, currentTerminology)].t,
      ...
      $results.fadeOut(500, () => {
        $results.html(enhanceResultsComponent(currentGroups)).fadeIn(300);
      });
    });
};
...
const wireUp = () => {
  ...
  $results
    ...
    .on('click', '.token', (event) => {
      ...
      $('.scrollable').scroll();
      $('.scrollable').animate({
        scrollTop: findPos(document.getElementById($(event.currentTarget).data('panel-id')))
          - findPos(document.getElementById('resultTables')),
      }, 200);
    })
...
const show = () => {
  defaultController(enhanceComponent);
  wireUp();
};
