...
const refreshSynonymUI = () => {
  const html = synonymComponent({ synonyms: Object.keys(currentCodeSet.s) });
  $synonym.include.list.html(html);
};

const refreshExclusionUI = () => {
  const html = synonymComponent({ synonyms: Object.keys(currentCodeSet.e) });
...
const show = () => {
  defaultController(validateComponent);
  wireup();
};
// params, state, url
